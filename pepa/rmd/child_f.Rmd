...
`r if (nf == 2 & (pv.gen < 0.05 | pv.int < 0.05)) {paste0("### ", {{i+1}}, ".2.3. LSD test for ", factors[1])}`

```{r}
if (nf == 2 & pv.gen < 0.05 & (pv.int > 0.05 | pe)) {
  y <- dfr[, trait.est]
  agricolae::LSD.test(y, dfr[, factors[1]], df.res, ms.res)$groups
}

if (nf == 2 & pv.int < 0.05) {
  for (j in 1:lc$nl[2]) {
    temp <- dfr[dfr[, factors[2]] == unique(dfr[, factors[2]])[j], ]
    y <- temp[, trait.est]
    print(paste("LSD test for", factors[2], unique(dfr[, factors[2]])[j]))
    print(agricolae::LSD.test(y, temp[, factors[1]], df.res, ms.res)$groups)
    cat("\n")
  }
}
```

`r if (nf == 2 & (pv.gen < 0.05 | pv.int < 0.05)) {paste0("### ", {{i+1}}, ".2.4. Tukey test for ", factors[1])}`

```{r}
if (nf == 2 & pv.gen < 0.05 & (pv.int > 0.05 | pe)) {
  y <- dfr[, trait.est]
  agricolae::HSD.test(y, dfr[, factors[1]], df.res, ms.res)$groups
}
...
if (nf == 2 & pv.int < 0.05) {
  for (j in 1:lc$nl[2]) {
    temp <- dfr[dfr[, factors[2]] == unique(dfr[, factors[2]])[j], ]
    y <- temp[, trait.est]
    print(paste("HSD Tukey test for", factors[2], unique(dfr[, factors[2]])[j]))
    print(agricolae::HSD.test(y, temp[, factors[1]], df.res, ms.res)$groups)
    cat("\n")
  }
}
```

`r if (nf == 2 & (pv.loc < 0.05 & pv.gen > 0.05 & pv.int > 0.05)) {paste0("### ", {{i+1}}, ".2.3. LSD test for ", factors[2])}`
`r if (nf == 2 & (pv.int < 0.05 | (pv.gen < 0.05 & pv.loc < 0.05))) {paste0("### ", {{i+1}}, ".2.5. LSD test for ", factors[2])}`

```{r}
if (nf == 2 & pv.loc < 0.05 & (pv.int > 0.05 | pe)) {
  y <- dfr[, trait.est]
  agricolae::LSD.test(y, dfr[, factors[2]], df.res, ms.res)$groups
}

if (nf == 2 & pv.int < 0.05) {
  for (j in 1:lc$nl[1]) {
    temp <- dfr[dfr[, factors[1]] == unique(dfr[, factors[1]])[j], ]
    y <- temp[, trait.est]
    print(paste("LSD test for ", factors[1], unique(dfr[, factors[1]])[j]))
    print(agricolae::LSD.test(y, temp[, factors[2]], df.res, ms.res)$groups)
    cat("\n")
  }
}
```

`r if (nf == 2 & (pv.loc < 0.05 & pv.gen > 0.05 & pv.int > 0.05)) {paste0("### ", {{i+1}}, ".2.4. Tukey test for ", factors[2])}`
`r if (nf == 2 & (pv.int < 0.05 | (pv.gen < 0.05 & pv.loc < 0.05))) {paste0("### ", {{i+1}}, ".2.6. Tukey test for ", factors[2])}`

```{r}
if (nf == 2 & pv.loc < 0.05 & (pv.int > 0.05 | pe)) {
  y <- dfr[, trait.est]
  agricolae::HSD.test(y, dfr[, factors[2]], df.res, ms.res)$groups
}
...
if (nf == 2 & pv.int < 0.05) {
  for (j in 1:lc$nl[1]) {
    temp <- dfr[dfr[, factors[1]] == unique(dfr[, factors[1]])[j], ]
    y <- temp[, trait.est]
    print(paste("HSD Tukey test for ", factors[1], unique(dfr[, factors[1]])[j]))
    print(agricolae::HSD.test(y, temp[, factors[2]], df.res, ms.res)$groups)
    cat("\n")
  }
}
```
